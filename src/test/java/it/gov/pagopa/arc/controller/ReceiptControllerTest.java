...
class ReceiptControllerTest {
    ...
    void givenFiltersWhenGetPagedDebtorReceiptsThenOk() {
        ...
        Mockito.when(receiptFacadeServiceMock.getPagedDebtorReceipts(brokerId, fiscalCode, debtorReceiptsFiltersDTO, pageRequest, loggedUser)).thenReturn(expectedResult);
        //when
    ...
    void whenGetReceiptDetailThenOk() {
        ...
        //when
        ResponseEntity<ReceiptDetailExtendedDTO> result = receiptController.getReceiptDetail(brokerId,organizationId,receiptId,fiscalCode);
        //then
        assertNotNull(result);
        assertEquals(HttpStatus.OK, result.getStatusCode());
        assertEquals(expectedResult, result.getBody());
